sap.ui.define(
  ...
  function (
    ...
    return Controller.extend(
      "com.sap4kids.resourcelocator.controller.MapView",
      {
        formatter: formatter,
        ...
        onInit: function () {
          ...
          UIComponent.getRouterFor(this)
            .getRoute("mapview")
            .attachPatternMatched(this._updateUI, this);
        },
        ...
        onAfterRendering: function () {
          ...
          oMap.attachEventOnce("ready", function () {
            this.map.addListener(
              "drag",
              oController.mapMoved.bind(this.map)(oController)
            );
            this.map.addListener(
              "dragend",
              oController.mapMoved.bind(this.map)(oController)
        ...
        mapMoved: function (oController) {
          ...
          return function () {
            ...
            oController._moveTimeout = setTimeout(
              function () {
        ...
        retrieveUserLocation: function () {
          ...
          if (navigator.geolocation) {
            //&& oStateModel && !oStateModel.getProperty("/locationFromUrl")) {
            navigator.geolocation.getCurrentPosition(
              function success(nativeLocation) {
        ...
        _getRequestParam: function () {
          ...
          const sResources =
            "''" + oStateModel.getProperty("/selectedResources").join("'',''");
          const sResourcesLocahost =
            "(ASSISTANCETYPEID eq " +
            oStateModel
              .getProperty("/selectedResources")
              .join(" or ASSISTANCETYPEID eq ") +
            ")";
        ...
        updateMarkerBinding: function (zoomType) {
          ...
          if (typeof zoomType === "object") {
            ...
            const oInfo = oRouter.getRouteInfoByHash(
          ...
          $.get("/public/map/SchoolOffersByType" + this._getRequestParam())
            .done(
              function (oResult) {
                const aTransformed = this.formatter.transformBackendData(
                  oResult.value
                );
        ...
        onPrintLegal: function (oEvent) {
          ...
          mywindow.document.write(
            oEvent.getSource().getParent().getContent()[0].getText()
          );
        ...
        onMarkerClick: function (oEvent) {
          ...
          if (oBindingContext) {
            ...
            const sInfo = this.formatter.markerInfoFormatter(
          ...
          this.getView()
            .byId("map")
            .getMarkers()
            .forEach(function (oMarker) {
              if (oEvent.getSource() !== oMarker || !Device.system.desktop) {
                //close all on mobile
        ...
        onAddressSearch: function (oEvent) {
          ...
          var currentHash = UIComponent.getRouterFor(this)
            .getHashChanger()
            .getHash();
          this.getView().byId("providerOverlay").setHidden(true);
          const oBundle = oEvent
            .getSource()
            .getModel("i18n")
            .getResourceBundle();
          ...
          MapUtils.search({
            ...
            function (results) {
              ...
              this.getView()
                .getModel("state")
                .setProperty("/determineLocation", false);
        ...
        getClosestMarkers: function () {
          ...
          while (aNeighbors.length < 10 && round < 100) {
            aNeighbors = this._getProviderMarkers().filter(function (oMarker) {
              ...
              const closeLat =
                Math.abs(oProviderContext.getProperty("lat") - location.lat) <
                round * 0.05;
              const closeLng =
                Math.abs(oProviderContext.getProperty("lng") - location.lng) <
                round * 0.05;
        ...
        onIOSAppOpen: function () {
          window.open(
            "https://apps.apple.com/us/app/genyouth-resource-finder/id1513648385?mt=8",
            "_blank"
          );
        },
        ...
        onFeedbackOpen: function () {
          window.open(
            "https://sapinsights.eu.qualtrics.com/jfe/form/SV_0N82QgYLLEE809T",
            "_blank"
          );
        },
        ...
        onTriggerNavigator: function (oEvent) {
          ...
          const sOrigin =
            "saddr=" +
            oStateModel.getProperty("/location/lat") +
            "," +
            oStateModel.getProperty("/location/lng") +
            "&";
          window.open(
            "http://maps.google.com/maps?" + sOrigin + "daddr=" + sDestination,
            "_blank"
          );
        },
      }
